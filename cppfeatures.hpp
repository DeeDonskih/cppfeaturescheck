#ifndef CXX_FEATURES_CHECKS_HPP
#define CXX_FEATURES_CHECKS_HPP
...
#endif
namespace cxxfeatures {
// Change these options to print_options out only necessary info.
    ...
    /**
     * @brief isSupported - checks is given feature supported
     * @param x - feature
     * @return
     */
    constexpr bool isSupported(const CompilerFeature &x) {
    ...
    inline void print_compiler_feature(const CompilerFeature &x) {
        ...
        std::string value{isSupported(x) ? x.value : "------"};
        if (value.back() == 'L') {
            value.pop_back();                      //~ 201603L -> 201603
        }
        // value.insert(4, 1, '-'); //~ 201603 -> 2016-03
        if ((print_options.supported_features && isSupported(x))
            or (print_options.unsupported_features && !isSupported(x))) {
    ...
    inline void show(char const *title, CompilerFeature (&features)[N]) {
        if (print_options.titles) {
            std::cout << '\n' << std::left << title << '\n';
        }
        if (print_options.sorted_by_value) {
    ...
    inline void print() {
        if (print_options.general_features) {
            show("C++ GENERAL", cxx);
        }
        if (print_options.cxx11 && print_options.core_features) {
            show("C++11 CORE", cxx11);
        }
        if (print_options.cxx14 && print_options.core_features) {
            show("C++14 CORE", cxx14);
        }
        if (print_options.cxx14 && print_options.lib_features) {
            show("C++14 LIB", cxx14lib);
        }
        if (print_options.cxx17 && print_options.core_features) {
            show("C++17 CORE", cxx17);
        }
        if (print_options.cxx17 && print_options.lib_features) {
            show("C++17 LIB", cxx17lib);
        }
        if (print_options.cxx20 && print_options.core_features) {
            show("C++20 CORE", cxx20);
        }
        if (print_options.cxx20 && print_options.lib_features) {
            show("C++20 LIB", cxx20lib);
        }
        if (print_options.cxx23 && print_options.core_features) {
            show("C++23 CORE", cxx23);
        }
        if (print_options.cxx23 && print_options.lib_features) {
            show("C++23 LIB", cxx23lib);
        }
        if (print_options.attributes) {
            show("ATTRIBUTES", attributes);
        }
    }

}
#endif // CXX_FEATURES_CHECKS_HPP
